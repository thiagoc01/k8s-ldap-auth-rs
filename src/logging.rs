...
use tracing_subscriber::{
    ...
    Layer,
    filter::Targets,
    fmt::{self, FmtContext, FormatEvent, FormatFields, format::Writer, time::FormatTime},
    layer::SubscriberExt,
    registry::LookupSpan,
...
pub fn format_error_chain(err: &dyn std::error::Error) -> String {
    ...
    while let Some(cause) = source {
        let indent = " ".repeat(depth * BASE_IDENT_NUM_COLUMNS as usize);
    ...
    if depth == 1 {
        format!("\n{}Caused by: {}", " ".repeat(BASE_IDENT_NUM_COLUMNS as usize), out)
    }
...
pub fn list_related_env_vars_application() {
    ...
    for (var, default_value) in POSSIBLE_ENV_VARS {
        ...
        if let Ok(_) = std::env::var(var) {
            ...
            tracing::debug!("Environment variable {} not found. Using default value: {}", var, default_value);
        }

        else {
            tracing::debug!("Environment variable {} not found. Using value from command line", var);
        }
    }
...
impl FormatTime for LocalTimer {
    fn format_time(&self, w: &mut Writer<'_>) -> std::fmt::Result {
        write!(w, "{}", chrono::Local::now().format("[%d/%m/%Y %H:%M:%S %Z]"))
    }
}

pub fn init_logging(level: &LogLevel, log_path: Option<&Path>) -> anyhow::Result<()> {
    ...
    let filter_crate = Targets::new().with_target(env!("CARGO_CRATE_NAME"), Level::DEBUG);
    ...
    let terminal_layer = fmt::layer()
        ...
        .with_writer(writer)
        .with_filter(EnvFilter::new(convert_to_tracing_levels(level).as_str()))
    ...
    match log_path {
        None => {
            ...
            tracing::warn!("No log file path provided. Using only stdout");
        }
        Some(path) => {
            match OpenOptions::new()
                ...
                Err(error) => {
                    ...
                    tracing::warn!("Could not initialize file log layer. Using only stdout. {}", format_error_chain(&error));
                }
    ...
    N: for<'a> FormatFields<'a> + 'static,
{
    fn format_event(
        ...
        let level = *event.metadata().level();
        write!(writer, " - {:1$} - ", level, BASE_IDENT_NUM_COLUMNS as usize)?;
...
mod tests {
    ...
    fn test_logging_format_error_chain_anyhow() {
        let error = anyhow::anyhow!("Root").context("Mid").context("Top");
    ...
    fn test_logging_format_error_chain_indentation_increases_per_depth() {
        ...
        for pair_neighbour_lines in leading_spaces.windows(2) {
            assert!(
                ...
                "Indentation should increase: {:?}", leading_spaces
            );
        }
    }
    ...
    fn test_logging_convert_to_tracing_levels(#[case] input: &LogLevel, #[case] expected: Level) {
        assert_eq!(convert_to_tracing_levels(input), expected);
    }
 
    #[rstest]
    #[serial]
    fn test_logging_print_banner_writes_version_to_file(get_file_path: PathBuf) -> Result<()> {
    ...
    #[rstest]
    #[serial]
    fn test_logging_print_banner_appends_on_multiple_calls(get_file_path: PathBuf) -> Result<()> {
    ...
    #[rstest]
    #[serial]
    fn test_logging_print_banner_file_contains_banner_constant(get_file_path: PathBuf) -> Result<()> {
        let _ = fs::remove_file(&get_file_path);
    ...
    #[test]
    fn test_logging_init_logging_stdout_only_does_not_return_error() -> Result<()> {
        init_logging(&LogLevel::INFO, None)
    }
 
    #[rstest]
    #[serial]
    fn test_logging_init_logging_with_file_path_does_not_return_error(get_file_path: PathBuf) -> Result<()> {
    ...
    #[test]
    fn test_logging_init_logging_invalid_file_path_does_not_error() -> Result<()> {
        // Should proceed using stdout only
        let nonexistent_path = PathBuf::from("/nonexistent/dir/test.log");
