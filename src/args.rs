...
impl Args {
        ...
        T: Into<std::ffi::OsString> + Clone,
    {
        ...
        let log_file_path = match Self::get_specific_arg(
            parser.clone(),
            "log-file-path",
            iter.clone(),
        ) {
        ...
        if load_env_res.is_err() {
            tracing::warn!(
                "Could not load environment file from the provided path neither from .env.\n\
                \tCaused by: {}",
        ...
        let socket_args = (
            ...
            Self::save_arg(&matches, "port", |port: String| {
                port.parse().unwrap_or_else(|error| {
                    println!(
                        "Could not parse the provided port number,\
        ...
        let tls_args = (
            ...
            Self::save_arg(&matches, "cacert", |cacert: String| {
                cacert
            }),
        );
        ...
        Ok(Args {
            ...
            ldap_args: LdapArgs::new(
                ...
                Self::save_arg(
                    &matches,
                    "ldap-bind-password",
                    |ldap_bind_password: String| ldap_bind_password,
                ),
                Self::save_arg(
                    &matches,
                    "ldap-search-base",
                    |ldap_search_base: String| ldap_search_base,
                ),
                Self::save_arg(
                    &matches,
                    "ldap-user-attr",
                    |ldap_user_param: String| ldap_user_param,
                ),
                Self::save_arg_many(
                    &matches,
                    "ldap-search-attrs",
                    |attrs: Vec<String>| attrs.join(","),
                ),
                Self::save_arg(
                    &matches,
                    "ldap-timeout-conn",
                    |ldap_timeout_conn: String| ldap_timeout_conn,
                ),
                matches
    ...
    fn save_arg_many<T, U>(
        matches: &ArgMatches,
        name: &str,
        process_arg: impl FnOnce(Vec<T>) -> U,
    ) -> U
    where
        T: Clone + Send + Sync + 'static,
        U: Clone + Send + Sync + 'static,
        ...
        command = command
            ...
            .arg(
                Arg::new("ldap-bind-password")
                    ...
                    .required(true)
                    .help(
                        "Password of the LDAP bind user that will search \
            ...
            .arg(
                Arg::new("ldap-search-attrs")
                    ...
                    .help(
                        "Attributes to retrieve from LDAP server. \
...
mod tests {
    ...
    #[rstest]
    #[serial]
    fn test_args_invalid_port_falls_back_to_7878(
        base_args: &'static Vec<&str>,
    ) {
        unsafe { clear_env() };
    ...
    #[serial]
    fn test_args_log_level_from_cli(
    ...
    fn test_args_log_file_path_from_cli() {
        ...
        assert_eq!(
            ...
            Some(r"/var/log/k8s-ldap-auth.log".to_string())
        );
    }
    ...
    fn test_args_env_file_loaded() -> Result<()> {
        ...
        let env_file_path = PathBuf::from(temp_dir())
            .join("k8s-ldap-auth-rs-test.env");
        ...
        assert!(
            log_file_path
                == Some("/var/log/k8s-ldap-auth.log".to_string())
        );
